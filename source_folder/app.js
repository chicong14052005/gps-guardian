...
// Biến lưu IP của ESP32 (Xem trong Serial Monitor khi ESP32 khởi động)
// Ví dụ: ESP32 in ra 192.168.1.15 thì điền vào đây hoặc tạo input nhập
let esp32Ip = "http://10.11.29.95";

let trackingInterval = null;


    // Hàm gọi liên tục mỗi 2 giây
async function startRealTimeTracking() {
    const ipInput = prompt("Nhập IP ESP32:", "10.11.29.95");
    if (!ipInput) return;
    const esp32Ip = `http://${ipInput}`;

    showToast("📡 Đang giám sát đa vùng an toàn...");
    if (trackingInterval) clearInterval(trackingInterval);

    trackingInterval = setInterval(async () => {
        try {
            const response = await fetch(`${esp32Ip}/gps`);
            const data = await response.json();

            if (data.lat && data.lat !== 0) {
                const espPos = [data.lat, data.lng];

                // 1. Cập nhật Marker vị trí thực trên bản đồ
                if (!testMarkerForRoute) {
                    testMarkerForRoute = L.marker(espPos, {
                        icon: L.divIcon({ html: '🛰️', className: 'esp-marker', iconSize: [30, 30] })
                    }).addTo(map);
                } else {
                    testMarkerForRoute.setLatLng(espPos);
                }

                // 2. Kiểm tra nút bấm vật lý từ ESP32
                if (data.alarm === true && !window.hardwareAlarmLocked) {
                    showAlert('🆘 KHẨN CẤP', 'Nút báo động trên ESP32 đã được kích hoạt!');
                    window.hardwareAlarmLocked = true; // Khóa để không hiện liên tục mỗi 2s
                } else if (data.alarm === false) {
                    window.hardwareAlarmLocked = false;
                }

                // 3. Tính toán xâm nhập đa vùng
                let isInside = false;
                let minDistance = Infinity;

                safeZones.forEach(zone => {
                    const distance = map.distance(zone.center, espPos);
                    if (distance < minDistance) minDistance = distance;

                    if (distance <= zone.radius) {
                        isInside = true;
                        zone.circle.setStyle({ color: '#10b981', fillOpacity: 0.3 }); // Xanh khi an toàn
                    } else {
                        zone.circle.setStyle({ color: '#10b981', fillOpacity: 0.1 });
                    }
                });

                // 4. Xử lý báo động ra khỏi vùng
                if (safeZones.length > 0 && !isInside) {
                    safeZones.forEach(z => z.circle.setStyle({ color: '#ef4444' })); // Đổi tất cả sang đỏ

                    if (!alertTriggered) {
                        alertTriggered = true;
                        showAlert('🚨 XÂM NHẬP!', `Thiết bị ra khỏi vùng an toàn! Cách tâm gần nhất ${Math.round(minDistance)}m`);
                        if (typeof sendRealAlertEmail === "function") sendRealAlertEmail('OUT_OF_ZONE');
                    }
                } else {
                    alertTriggered = false; // Reset trạng thái khi quay lại vùng an toàn
                }
    ...
    function clearAllSafeZones() {
        safeZones.forEach(zone => map.removeLayer(zone.circle));
        safeZones = [];
        alertTriggered = false;
        showToast("🧹 Đã xóa sạch các vùng an toàn");
    }
// Gán chức năng này vào một nút nào đó, ví dụ nút "Test theo đường" để test
// document.getElementById('testRouteBtn').onclick = startRealTimeTracking;

async function manualIntrusionCheck() {
    // 1. Xác định địa chỉ ESP32
    if (!window.esp32Ip) {
        const ip = prompt("Nhập IP ESP32 để truy xuất vị trí thực:", "192.168.1.15");
        if (!ip) return;
        window.esp32Ip = `http://${ip}`;
    }

    showToast("🛰️ Đang lấy tọa độ thực tế từ ESP32...");

    try {
        // 2. Gọi lệnh lấy GPS từ phần cứng
        const response = await fetch(`${window.esp32Ip}/gps`, { mode: 'cors' });
        const data = await response.json();

        // 3. Kiểm tra dữ liệu ESP32 trả về
        if (data.lat && data.lat !== 0) {
            const espLocation = [data.lat, data.lng]; // ĐÂY MỚI LÀ TỌA ĐỘ THỰC
            
            // Di chuyển bản đồ đến vị trí ESP32 đang đứng
            map.panTo(espLocation);
            
            // Vẽ/Cập nhật marker của ESP32
            if (!testMarkerForRoute) {
                testMarkerForRoute = L.marker(espLocation, {
                    icon: L.divIcon({ html: '🛰️', className: 'esp-marker', iconSize: [30, 30] })
                }).addTo(map);
            } else {
                testMarkerForRoute.setLatLng(espLocation);
            }

            // 4. THỰC HIỆN TÍNH TOÁN XÂM NHẬP
            if (safeZones.length === 0) {
                showAlert('ℹ️ Chưa có vùng an toàn', 'Hãy CHUỘT PHẢI lên bản đồ để tạo ít nhất một vùng an toàn trước khi kiểm tra!');
                return;
            }
            ...
            safeZones.forEach((zone, index) => {
                ...
                const distance = map.distance(zone.center, espLocation);
                if (distance < closestDistance) closestDistance = distance;

                if (distance <= zone.radius) {
                    isInside = true;
                    zone.circle.setStyle({ color: '#10b981', fillOpacity: 0.4 }); // Xanh khi an toàn
                } else {
                    zone.circle.setStyle({ color: '#ef4444', fillOpacity: 0.2 }); // Đỏ khi ngoài vùng
                }
            });

            // 5. HIỂN THỊ KẾT QUẢ
            if (isInside) {
                showToast(`✅ AN TOÀN! Thiết bị cách tâm vùng gần nhất ${Math.round(closestDistance)}m`);
            } else {
                showAlert('🚨 PHÁT HIỆN XÂM NHẬP!', `Thiết bị đang nằm NGOÀI tất cả các vùng an toàn!\nKhoảng cách: ${Math.round(closestDistance)}m`);
                
                // Gửi email báo động ngay lập tức
                if (typeof sendRealAlertEmail === "function") {
                    sendRealAlertEmail('OUT_OF_ZONE');
                }
            }
        } else {
            showAlert('⚠️ Lỗi GPS', 'ESP32 chưa bắt được vệ tinh (Tọa độ trả về = 0). Hãy đưa thiết bị ra ngoài trời!');
        }
    } catch (error) {
        console.error("Lỗi kết nối:", error);
        showAlert('❌ Lỗi kết nối', 'Không thể kết nối tới ESP32. Hãy kiểm tra IP và WiFi!');
    }
}